...
Deterministic tests cover compatibility, loader/distribution matching, prerelease channels, readiness, scoped decisions, SQLite transactions/migrations, cancellation, stale contexts, partial failure, rate limiting, and save failure. The live API test is skipped by default. To run it explicitly:
...
- Database: `%LOCALAPPDATA%\TheModMenagerie\menagerie.db`
- Diagnostic logs: `%LOCALAPPDATA%\TheModMenagerie\logs\yyyy-MM-dd.log`
...
- Close the app before backing up or restoring. Copy the entire `TheModMenagerie` data directory to a safe location. Restore while the app is closed. Do not replace a live database.
...
- Dependency analysis, `.mrpack` import, compatibility history, deterministic ranges, other providers, accounts, sync, notifications, and game-file management are outside this MVP.
...
Rerun the same script after publishing updates. This is a local-folder installation, not a registered installer or automatic updater. Your database remains separately stored in `%LOCALAPPDATA%\TheModMenagerie`. Removing the program folder and Start menu shortcut does not remove that database.
