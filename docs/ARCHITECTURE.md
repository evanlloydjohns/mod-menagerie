...
The same release must explicitly contain the exact target and required loader, or have a deliberately entered evidenced range rule for that target while still explicitly listing the loader. Datapacks require `datapack`, independent of the collection's mod loader. Stable (`release`) wins over beta, then alpha; within a channel choose descending publication time and ordinal ascending ID. Unrecognized or incomplete required evidence yields Unknown conservatively. A complete successful empty match yields Not detected, not proof of incompatibility. Lifecycle is orthogonal.
...
Refresh saves each project independently in a transaction. Failure stores Unknown and preserves last successful evidence. Removed memberships are checked before save so late results cannot resurrect them. Old-context responses write only the captured old scope. Evaluation records retain the latest outcome per scope; separate bounded history snapshots record observations over time. Shared metadata/version caches are reusable across modpacks.
...
SQLite tables: packs, projects, memberships, versions, evaluations, overrides, preferences, range_rules, history. Identity, many-to-many membership, uniqueness, and cascading membership/override cleanup are relational; descriptive payloads are JSON. This keeps provider metadata flexible without an ORM or dozens of join tables. User-authored and cache records stay separate. Explicit transactional migrations upgrade schema 1 to 2 while preserving existing records. Newer schemas are refused rather than destroyed. Foreign keys protect local state, and exceptions from durable writes are never reported as success.
...
Phase 2 adds `DependencyAnalysis` and `PackImport` in Application, numeric `VersionPatterns` in Domain, bounded `.mrpack` parsing and schema-2 persistence in Infrastructure, and four focused desktop dialogs. The new schema adds range rules and bounded history without replacing existing user records. Dependency metadata is mapped per release; exact version and hash endpoints supplement the existing project cache. Dependency reports are cached with a fingerprint of context, membership, decisions and evaluated releases, so changes invalidate the report. See [Phase 2](PHASE2.md) for aggregate semantics and limits.

The default data directory is now %USERPROFILE%\.modmenagerie. AppData was redirected when launched by a packaged host, producing different database views from a normal Explorer launch. The profile-root directory avoids that redirection. On first run with no destination database, SQLite backup migrates the visible legacy database atomically and preserves its original. MOD_MENAGERIE_DATA remains an explicit override. The footer tooltip shows the active data directory.
